/**
 * Expects the values in a specified column of a table to not match a given regular expression.
 *
 ...
 * @param {string} params.regex - The regular expression pattern to match against the column values.
 * @returns {Object} - The assertion object to check if all records match the regex.
 */
...
module.exports = (params) => {
  ...
  const sqlQuery = ctx => `
    ...
    FROM
      ${ctx.ref(source)}
    WHERE
      REGEXP_CONTAINS(${column}, r'${regex}')
  `;
